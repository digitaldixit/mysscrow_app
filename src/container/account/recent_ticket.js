...
import { getRecentTickets } from './account_action';
...
class AccountTicket extends Component {
  ...
  componentWillMount() {
    ...
    this.props.getRecentTickets(filterdata, function(err,result) {
      if(result) {
  ...
  handlePageChange(pageNumber) {
    ...
    this.props.getTickets(filterdata, function(err, result) {
      if(result) {
            ...
            <div className="card-ticket">
              <div className="row">
                <div className="col-sm-3 col-3">
                  <div className="colored-side" style={{"backgroundColor":ticket.status_color}}></div>
                ...
                <div className="col-sm-9 col-9">
                  <div className=" row product-row-1 ticket-from">
                          ...
                          <div className="col-lg-3 col-md-3 col-sm-3 col-xs-3 col-3 price  text-right diate pull-right">
                            <Link className="btn-Status btn btn-success btn-sm pull-right " style={{"backgroundColor":ticket.status_color}}>{ticket.status_name}</Link>
                          </div>
                  </div>
                  <hr/>
                    <div className=" row product-row-1 ticket-from">
  ...
  render() {
    const { ticket_list ,settings} = this.props;
                          ...
                          <div className="text-center mt-2 mb-2">
                              <div className="card">
                                  <div className="card-body p-2">
                                    <div className="no-result"><h5>No Recent Ticket found!</h5></div>
                                  </div>
                              </div>
                        </div>
                      </div>
...
function mapStateToProps(state) {
  console.log("--- state.account.ticket_list", state.account.ticket_list)
  return {
...
export default connect(mapStateToProps, { getRecentTickets })(AccountTicket);
