...
class AccountTicket extends Component {
  ...
  refresh() {
    this.setState({isLoading: true, currentPage: 1, search_text: '' });
    var _this = this;
    var page  = 1;
                  ...
                  <hr/>
                    <div className=" row product-row-1 ticket-from">
                            <div className="col-lg-12 col-md-12 col-sm-12 col-xs-12 col-12 text-left ticket-service">
                              ...
                              ticket.description
                              ?
                              <p className="m-0"><span className="btn-outine">{ticket.description}</span></p>
                              :
                              <p className="m-0"><span className="btn-outine">no data </span></p>
                            }
                            ...
                            ticket.address
                              ...
                              ?
                              <p className="m-0"><i className="fa fa-map-marker fa-lg mr-1"></i><span className="ticket-title">{ticket.address}</span></p>
                              :
                              <p className="m-0"><span className="ticket-title">no details</span></p>
                            }
                            </div>
  ...
  renderTicketStatus(statues, currentTicketStatus) {
       if(statues !== undefined) {
         return statues.map((status)=>{
             return(
                 <option   key={"tickets.all.status."+status.service_status_id} value={status.service_status_id}>{status.name}</option>
             );
         });
       }
     }
   renderTicketSortBy(statues, currentTicketStatus){
           return (
             <div className="category-sortby mb-1">
               <div className="select-style">
               <select onChange={ (e) => this.applyCategorySortBy('filter_ticket_status', e.target.value)}>
                   <option value="">--Select Ticket Status--</option>
                 {this.renderTicketStatus(statues, currentTicketStatus)}
                 </select>
               </div>
             </div>
           )
  ...
  <div className="account-layout account-box">
          ...
          <div className="container">
                ...
                <strong>Active Ticket List</strong>
            ...
            {
              filter_ticket_status != undefined && filter_ticket_status.length > 0
              ?
                <div>
                  {this.renderTicketSortBy(filter_ticket_status, currentTicketStatus)}
                </div>
              :
                null
            }
            </div>
            {/* <div className="col-sm-6 col-sm-6  text-right pull-right">
                    {
